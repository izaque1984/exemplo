...
uses UnDmpdv, UnFrmlocalizarvenda, UnFrmcancelarnota, UnFrmpdv;
...
If DMLOJA.TBVENDAPROTOCOLONOTA.AsString < '0' then // condição
DBGrid1.Canvas.Font.Color:= clGreen;  // coloque aqui a cor desejada
DBGrid1.DefaultDrawDataCell(Rect, DBGrid1.columns[datacol].field, State);

If DMLOJA.TBVENDASTATUSDANOTA.AsString = 'Rejeitado' then // condição
...
procedure TFRMMTCVENDAS.SpeedButton2Click(Sender: TObject);
begin

  if dmloja.TBCONFIGUF.AsString = 'SI' then

  begin


  iRetorno := iCFReImprimir_NFCe_Daruma('', '', '');
  frmpdv.TrataRetorno_NFCe(iRetorno);
    ...
    begin
        Application.MessageBox('Sistema Incompativel com impressora instalada!','Informação do Sistema',Mb_IconInformation);
...
begin
  if Application.MessageBox('Confirma a Exclusão da Venda??','Comfirmação',MB_YESNO + MB_ICONQUESTION)= IDYES then
...
//--------------------------DELETA DETVENDA E VOLTA PARA ESTOQUE----------------------------
  ...
  Application.MessageBox('Venda NFC-e Excluida!','Informação do Sistema',Mb_IconInformation);
