...
If DMLOJA.TBESTOQUECFOP.AsString = '5405' then // condição
...
begin
 if Application.MessageBox('Confirma a Exclusão do Produto?','Comfirmação',MB_YESNO + MB_ICONQUESTION)= IDYES then

  begin
   DMLOJA.TBESTOQUE.Delete;
   Application.MessageBox('Produto Deletado do Estoque!','Informação do Sistema',Mb_IconInformation);


  end
else
 Begin
  Application.MessageBox('Exclusão cancelada!','Informação do Sistema',Mb_IconInformation);
  Exit
 end;
end;
